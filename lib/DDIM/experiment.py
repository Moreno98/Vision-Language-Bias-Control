...
from lib.dist_utils import *
from lib.lmdb_writer import *
...
class LitModel(pl.LightningModule):
    ...
    def normalize(self, cond):
        cond = (cond - self.conds_mean.cuda()) / self.conds_std.to(
            self.device)
        return cond

    def denormalize(self, cond):
        cond = (cond * self.conds_std.cuda()) + self.conds_mean.to(
            self.device)
        return cond
    ...
    def sample_latent(self, 
        ...
        if conf.train_mode.is_latent_diffusion():
            ...
            if conf.train_mode == TrainMode.latent_diffusion:
                latent_noise = torch.randn(length, conf.style_ch, device=device)
            else:
            ...
            cond = latent_sampler.sample(
                model=self.ema_model.latent_net,
                noise=latent_noise,
    ...
    def sample(self, N, device, T=None, T_latent=None, noise=None):
        if T is None:
        ...
        pred_img, cond = render_uncondition(
            self.conf,
            self.ema_model,
            noise,
            sampler=sampler,
        ...
        return pred_img, cond, noise

    def render(self, noise, cond=None, T=None, train = False):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        with writer:
            for batch in tqdm(loader, total=len(loader), desc='infer'):
                with torch.no_grad():
                    ...
                    cond = model.encoder(batch['img'].cuda())
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                if self.conf.latent_znormalize:
                    cond = (cond - self.conds_mean.to(
                        self.device)) / self.conds_std.cuda()
            else:
