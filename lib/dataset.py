...
class RAF_DB_Inverted(Dataset):
    def __init__(
        ...
        self.noise_latents_path = os.path.join(path, "latents", "noises")
        self.semantic_latents_path = os.path.join(path, "latents", "semantic_codes")
        self.transform = transform
        self.images = [image for image in os.listdir(self.images_path) if "train" in image]
    ...
    def __getitem__(self, index):
        ...
        noise = torch.load(
            os.path.join(self.noise_latents_path, f"noise_{image_name}.pt"), 
            map_location="cpu"
        )
        semantic_latent = torch.load(
            os.path.join(self.semantic_latents_path, f"semantic_codes_{image_name}.pt"), 
            map_location="cpu"
        )
        image = self.transform(image)
        return image, noise, semantic_latent
...
class Generated_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        semantic_latent = torch.load(
            os.path.join(self.semantic_latents_path, f"semantic_code_{image_name}.pt"), 
            map_location="cpu"
        )
        image = self.transform(image)
        return image, noise, semantic_latent